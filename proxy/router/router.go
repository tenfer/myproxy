...
func parseRule(db string, cfg *config.ShardConfig) (*Rule, error) {
	...
	r.DB = db
	r.Table = cfg.Table
	r.TableNoPrefix = cfg.TableNoPrefix
	r.TableNoFill = cfg.TableNoFill
	r.Key = cfg.Key
	r.Type = cfg.Type
	...
	case NuomiRuleType:
		var sumTables, tableNo, nodeNum int
		nodeNum = len(cfg.Locations)
		if nodeNum != len(r.Nodes) {
			return nil, errors.ErrLocationsCount
		}
		...
		for i := 0; i < len(cfg.Locations); i++ {
			for j := 0; j < cfg.Locations[i]; j++ {
				tableNo = j + sumTables
				r.SubTableIndexs = append(r.SubTableIndexs, tableNo)
				//node编号=表编号 % 节点数量
				r.TableToNode[tableNo] = tableNo % nodeNum
			}
			sumTables += cfg.Locations[i]
		}
...
func (r *Router) buildSelectPlan(statement sqlparser.Statement) (*Plan, error) {
	...
	fmt.Println("plan.Rule:", plan.Rule)
	where = stmt.Where

	if where != nil {
		plan.Criteria = where.Expr //路由条件
		fmt.Println("plan.Criteria:", plan.Criteria)
...
func (r *Router) rewriteSelectSql(plan *Plan, node *sqlparser.Select, tableIndex int) string {
	...
	for _, expr := range node.SelectExprs {
		...
		case *sqlparser.StarExpr:
			...
			if string(v.TableName) == plan.Rule.Table {
				format := "%s%s" + r.getTableAppendFormat(plan) + ".*"
		...
		case *sqlparser.NonStarExpr:
			...
			if colName, ok := v.Expr.(*sqlparser.ColName); ok {
				if string(colName.Qualifier) == plan.Rule.Table {
					format := "%s%s" + r.getTableAppendFormat(plan) + ".%s"
					fmt.Fprintf(buf, format,
						prefix,
						plan.Rule.Table,
						tableIndex,
	...
	case *sqlparser.AliasedTableExpr:
		if len(v.As) != 0 {
			format := "%s" + r.getTableAppendFormat(plan) + "as %s"
			fmt.Fprintf(buf, format,
			...
			format := "%s" + r.getTableAppendFormat(plan)
			fmt.Fprintf(buf, format,
				sqlparser.String(v.Expr),
				tableIndex,
			)
...
func (r *Router) generateInsertSql(plan *Plan, stmt sqlparser.Statement) error {
	...
	if len(plan.RouteTableIndexs) == 0 {
		...
		for i := 0; i < tableCount; i++ {
			...
			fmt.Fprintf(buf, r.getTableAppendFormat(plan), plan.RouteTableIndexs[i])
			buf.Fprintf("%v %v%v",
				node.Columns,
...
func (r *Router) getTableAppendFormat(plan *Plan) string {
	format := plan.Rule.TableNoPrefix + "_"
	if plan.Rule.TableNoFill == 0 {
		format += "%d"
	} else {
		format += "%0" + strconv.Itoa(plan.Rule.TableNoFill) + "d"
	}
	return format
}
